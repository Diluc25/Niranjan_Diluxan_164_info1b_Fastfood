...
@app.route("/menu_afficher/<string:order_by>/<int:id_genre_sel>", methods=['GET', 'POST'])
def menu_afficher(order_by, id_genre_sel):
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_genre_sel == 0:
                    strsql_menu_afficher = """SELECT * FROM t_client inner join t_commande_passer on t_client.ID_Client = t_commande_passer.ID_Commande"""
                    mc_afficher.execute(strsql_menu_afficher)
                elif order_by == "ASC":
                    ...
                    strsql_menu_afficher = """SELECT *  FROM t_client WHERE ID_Client = %(value_id_genre_selected)s"""

                    mc_afficher.execute(strsql_menu_afficher, valeur_id_genre_selected_dictionnaire)
                else:
                    strsql_menu_afficher = """SELECT *  FROM t_client ORDER BY id_client DESC"""
        ...
        except Exception as Exception_menu_afficher:
            raise ExceptionGenresAfficher(f"fichier : {Path(__file__).name}  ;  "
                                          f"{menu_afficher.__name__} ; "
                                          f"{Exception_menu_afficher}")

    # Envoie la page "HTML" au serveur.
    return render_template("menu/menu_afficher.html", data=data_genres)
...
@app.route("/menu_delete", methods=['GET', 'POST'])
def menu_delete_wtf():
